...
namespace SoulsFormats.Util
{
    ...
    internal class NativeRsa
    {
        ...
        private unsafe static extern int get_decrypted_size(int encryptedLen, [In] byte[] key, out byte *err);

        [DllImport("bhd5_decrypt_rust.dll", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
        private unsafe static extern int decrypt([In] byte[] data, int dataLen, byte[] output, int outputSize, [In] byte[] key, int numThreads, out byte* err);
        
        [DllImport("bhd5_decrypt_rust.dll", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
        private unsafe static extern void free_error(byte* err);
        ...
        /// </summary>
        public static byte[] Decrypt(byte[] encrypted, string key, int decryptedSize, int numThreads)
        {
            if (numThreads < 1) throw new Exception("Cannot use 0 threads");
            var ans = new byte[decryptedSize];
            unsafe {
                int realDecSize = decrypt(encrypted, encrypted.Length, ans, decryptedSize, Encoding.UTF8.GetBytes(key), numThreads, out var err);
                CheckError(err);
        ...
        /// </summary>
        public static byte[] Decrypt(byte[] encrypted, string key, int numThreads)
        {
            unsafe {
